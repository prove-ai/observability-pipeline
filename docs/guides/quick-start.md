...
### Important Notice
This containerized bundle and the associated observability pipeline are provided on an "as-is" and "as-available" basis. It is intended for testing and evaluation purposes only and is not designed for mission-critical production use. Features may be modified, discontinued, or contain errors.
...
- **Infrastructure Control**: You are solely responsible for the installation, resource management, and security of the OpenTelemetry (OTel) and Prometheus containers within your own infrastructure.
- **No Data Persistence**: Prove AI does not host, back up, or store the telemetry data generated by this bundle. You are responsible for configuring persistent storage and data backups for your Prometheus instance.
- **No Warranty for Local Performance**: Prove AI is not liable for any system instability, resource exhaustion, or data loss occurring on your local host as a result of running these containers.
- **Prohibited Use**: This observability pipeline must not be used in connection with any high-risk activities where a failure of telemetry data could lead to death, personal injury, or severe environmental damage.

## Next Steps
For a full guide with troubleshooting and validation, check out our [vLLM Observability Guide](vllm-guide.md)
